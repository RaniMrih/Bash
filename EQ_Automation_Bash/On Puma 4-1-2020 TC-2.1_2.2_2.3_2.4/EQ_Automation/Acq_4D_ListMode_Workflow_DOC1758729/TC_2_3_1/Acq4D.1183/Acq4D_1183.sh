...
echo
echo ${GREEN}:::::::::::::::::::::::::::::::::::::::::::::::::: Running Step 1183, Please wait... ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::${reset}
echo
#----------------------------------------- functions --------------------------------------------
YesNoFunction () {
 # echo $1
...
for i in {1..3}
 do
   ReplayDuration2_3_1=$(grep -m$i "Frame Duration"  $S2_3_1Sino | tail -n1 | sed -n -e 's/^.*   Frame Duration   =//p'| sed 's|\(.*\) milliseconds.*|\1|' )
   ...
   result=$(echo "$ReplayDuration2_3_1 $OriginDuration2_1" |  awk '{printf "%f", $1 / $2}')
...
for i in {1..3}
 do
   ReplayTotalPrompts2_3_1=$(egrep -m$i ' totalPrompts ' $S2_3_1Sino | tail -n1 | sed 's|\(.*\)   totalPrompts     = *|\1|')
   ReplayDuration2_3_1=$(grep -m$i "Frame Duration"  $S2_3_1Sino | tail -n1 | sed -n -e 's/^.*   Frame Duration   =//p'| sed 's|\(.*\) milliseconds.*|\1|' )
   OriginDuration2_1=$(grep -m$i "Frame Duration"  $S2_1Sino | tail -n1 | sed -n -e 's/^.*   Frame Duration   =//p'| sed 's|\(.*\) milliseconds.*|\1|' )
   OriginPrompts2_1=$(egrep -m$i ' totalPrompts ' $S2_1Sino  | tail -n1 | sed 's|\(.*\)   totalPrompts     = *|\1|')
   result=$(echo "$ReplayDuration2_3_1 $OriginDuration2_1" |  awk '{printf "%f", $1 / $2}')
 ...
 #----------------------------------------
   ...
   H_Limit=$(printf "%.0f\n" $H_Limit)
   L_Limit=$(printf "%.0f\n" $L_Limit)
   if ((ReplayTotalPrompts2_3_1 > H_Limit )) || ((ReplayTotalPrompts2_3_1 < L_Limit ));then
   within=false
   fi
done
...
for i in {1..3}
  do
   ...
   result=$(echo "$ReplayDuration2_3_1 $OriginDuration2_1" |  awk '{printf "%f", $1 / $2}')
   extrapolated=$(echo "$result $OriginPrompts2_1" | awk '{printf "%f", $1 * $2}')
   diff=$(echo "$extrapolated $ReplayTotalPrompts2_3_1" | awk '{printf "%f", $1 - $2}')
...
#------- Dwell sum within 1500 frame duration ------------------
...
for i in {0..9}
 do
   ...
   FrameDuration2_3_1=$(grep -m$j "Frame Duration"  $S2_3_1Sino | tail -n1 | sed -n -e 's/^.*   Frame Duration   =//p' | sed 's|\(.*\) milliseconds.*|\1|')
   Max_F_Duration=$(expr $FrameDuration2_3_1 + 1500)   
   Min_F_Duration=$(expr $FrameDuration2_3_1 - 1500)
...
for j in {1..3}
 do
 A_Triggers=$(grep -m1 "   acceptedTriggers =" $S2_3_1Sino | tail -n1 | sed -n -e 's/^.*   acceptedTriggers = //p')
...
chmod +x /usr/g/ctuser/EQ_Automation/Acq_4D_ListMode_Workflow_DOC1758729/TC_2_3_1/Acq4D.1185/Acq4D_1185.sh
"/usr/g/ctuser/EQ_Automation/Acq_4D_ListMode_Workflow_DOC1758729/TC_2_3_1/Acq4D.1185/Acq4D_1185.sh"
