...
echo
echo ${GREEN}:::::::::::::::::::::::::::::::::::::::::::::::::: Running Step 1182, Please wait... ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::${reset}
echo
#----------------------------------------- functions --------------------------------------------
YesNoFunction () {
 # echo $1
...
for i in {1..3}
 do
   EventLoss1=$( grep -m$i "Event Loss due to Sorter being bandwidth limited"  $S2_3_1Sino | tail -n1 | sed 's|\(.*\): Event Loss due to Sorter being bandwidth limited.*|\1|')
   TotalCoinc1=$( grep -m$i "Total coincidence events transmitted to the Sorter"  $S2_3_1Sino | tail -n1 | sed 's|\(.*\): Total coincidence events transmitted to the Sorter.*|\1|')
...
for i in {1..3}
 do
   TotalCoinc=$( grep -m$i "Total coincidence events transmitted to the Sorter"  $S2_3_1Sino | tail -n1 | sed 's|\(.*\): Total coincidence events transmitted to the Sorter.*|\1|')
   echo ${BLUE}"bed $i :"${reset}
   echo $TotalCoinc
done
echo
...
for i in {1..3}
 do
   EventLoss=$( grep -m$i "Event Loss due to Sorter being bandwidth limited"  $S2_3_1Sino | tail -n1 | sed 's|\(.*\): Event Loss due to Sorter being bandwidth limited.*|\1|')
   echo ${BLUE}"bed $i :"${reset}
   echo $EventLoss
done
echo
...
chmod +x /usr/g/ctuser/EQ_Automation/Acq_4D_ListMode_Workflow_DOC1758729/TC_2_3_1/Acq4D.1183/Acq4D_1183.sh
"/usr/g/ctuser/EQ_Automation/Acq_4D_ListMode_Workflow_DOC1758729/TC_2_3_1/Acq4D.1183/Acq4D_1183.sh"
