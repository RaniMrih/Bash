...
echo
sleep 1
echo ${GREEN}:::::::::::::::::::::::::::::::::::::::::::::::::: Running Step 1417, Please wait... ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::${reset}
echo
#----------------------------------------- functions --------------------------------------------
YesNoFunction () {
 # echo $1
...
for i in {1..3}
 do
   ScanStart2_3_2=$(grep -m$i "Scan Start Time" $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*   Scan Start Time =//p' | grep -o '[0-9][0-9]:[0-9][0-9]:[0-9][0-9]')
   ...
   t=$(date -d "20121012 ${ScanStart2_3_2}" +%s)
...
for i in {1..3}
 do
   FrameStart2_3_2=$(grep -m$i "Frame Start Time =" $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*   Frame Start Time =//p' | grep -o '[0-9][0-9]:[0-9][0-9]:[0-9][0-9]')
   FrameStart2_1=$(grep -m$i "Frame Start Time =" $S2_1Sino | tail -n1 | sed -n -e 's/^.*   Frame Start Time =//p' | grep -o '[0-9][0-9]:[0-9][0-9]:[0-9][0-9]')
  #compare sec and display
   t=$(date -d "20121012 ${FrameStart2_3_2}" +%s)
...
for i in {1..3}
 do
   FrameDuration2_3_2=$(grep -m$i "Frame Duration"  $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*   Frame Duration   =//p' | sed 's|\(.*\) milliseconds.*|\1|')
   FrameDuration2_1=$(grep -m$i "Frame Duration"  $S2_1Sino | tail -n1 | sed -n -e 's/^.*   Frame Duration   =//p' | sed 's|\(.*\) milliseconds.*|\1|')
   t1=$(expr $FrameDuration2_3_2 / 1000)
...
for i in {1..3}
 do
   ScanMode=$(grep -m$i "scanMode" $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*acqParams.RDFAcqRxScanParams.scanMode: //p')
   if [[ "$ScanMode" != 2 ]];then
   NumberTwo=false
   fi
done
...
for i in {1..3}
 do
   ScanStart=$(grep -m$i "Scan Start Time" $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*   Scan Start Time =//p')
...
for i in {1..3}
 do
   FrameStart=$(grep -m$i "Frame Start Time =" $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*   Frame Start Time =//p')
   echo ${BLUE}"bed $i :"${reset}
   echo $FrameStart
done
echo
...
for i in {1..3}
 do
   FrameDuration=$(grep -m$i "Frame Duration"  $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*   Frame Duration   =//p')
   echo ${BLUE}"bed $i :"${reset}
   echo $FrameDuration
done
echo
...
for i in {1..3}
 do
   A_Triggers=$(egrep -m$i "   acceptedTriggers =" $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*   acceptedTriggers =//p')
   echo ${BLUE}"bed $i :"${reset}
   echo $A_Triggers
...
for i in {1..3}
 do
   R_Triggers=$(egrep -m$i "   rejectedTriggers" $S2_3_2Sino | tail -n1 | sed -n -e 's/^.*   rejectedTriggers =//p')
   echo ${BLUE}"bed $i :"${reset}
   echo $R_Triggers
...
chmod +x /usr/g/ctuser/EQ_Automation/Acq_4D_ListMode_Workflow_DOC1758729/TC_2_3_2/Acq4D.1418/Acq4D_1418.sh
"/usr/g/ctuser/EQ_Automation/Acq_4D_ListMode_Workflow_DOC1758729/TC_2_3_2/Acq4D.1418/Acq4D_1418.sh"
